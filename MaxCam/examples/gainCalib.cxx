...
void
plotResult(int runno) {

    // compute energy loss in bin range used to measure light intensity
    MaxCamSRIM he("SRIM_He_in_CF4_100Torr");
    he.setPressure(150); // Torr
    float d1=10;
    float d2=d1+11*8/16.4;
    float de=he.calcEnergyLoss(5350,d1,d2); 
    
    TChain res("res");
    TString fileName="RESULTS/results_run"; fileName+=runno; fileName+=".root";
    res.Add(fileName);

    TString com="segment.E()/"; com+=de; com+=":setup.time/3600/24";
    res.Draw(com,
             "segment.E()>0&&segment.E()<1e4&&segment.Mean()>280&&segment.Mean()<330&&segment.W()>4&&segment.W()<10");
    TGraph *graph = (TGraph*)gPad->GetPrimitive("Graph")->Clone("graph"); 

    
    gStyle->SetOptFit(1);
    TH1F* h=MaxCamImageTools::makeProfile( graph,  10, 0, 1.1, 2000/de, 7000/de);
    TF1 *fun=new TF1("fun","[0]*(1+[1]*x)"); fun->SetParNames("G","#DeltaG/G");
    h->Fit("fun");
    h->SetXTitle("Days");
    h->SetMinimum(0); h->SetMaximum(25);
    h->Draw();
}
