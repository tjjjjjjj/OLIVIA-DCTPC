...
{
  if (En <= 0) En = 1.e-20;
  return pow(2*En/(3.8e6 / 9.e22),0.5); //magic number is alpha particle mass over c^2.
}
  
int step(double& En, double dx, double& t, double dt, double a1, double a2, int i, double vd, int jmax, int jmax_init, vector<double>& Q, vector<double>& fRecoilZ, vector<double>& fRecoilEn, TRandom3* rand) //Computes when the electrons hit the mesh.
{
  ...
  tSD = timenoise(a1,a2,sz); //standard deviation
  jD = j(t,sz,vd,dt);
  int Nsamples = 50;
...
void decay(double Tr, double Td, double dt, vector<double>& Q, vector<double>& R, int jmax) //simulates rise, decay times
{
  ...
  cout << "DECAY CONSTANT IS " << DecayConst << endl;
...
{ 
  ...
  const double Tr = 5.e-9; //rise time
  const double Td = 5000.e-9; //decay time
  const double vd = 1.6e7; //drift velocity
  const double SD = 3.e0; //noise strength
  const double rate = 2.5e8; //sampling rate
  const double dt = 1./rate; //time step
  const double a1 = 5; //time noise strength (constant)
  const double a2 = 2.5; //(position dependent)

  int need_space = 0;
 
  const int jmax_init = 100000; //size of memory allocated when vectors need more space. Should be much larger than random noise.
 ...
 {
      need_space = step(En,dx,t,dt,a1,a2,i,vd,jmax,jmax_init,Q,fRecoilZ,fRecoilEn,rand);
      if(need_space == 1) //allocates extra space to vectors if necessary
	{
      ...
      //ifin = i; Useful for plotting fRecoil in the scope for debugging
    }
  ...
  //for (int j = j_init>100 ? j_init-100 : 1;j<j_init+1000;j++)
  //new TCanvas("","",2000,2000);
  int plotmax = scope(0)->getRecordLength();
  //int plotmax = ifin;
for (int j = 1; j < plotmax; j++) 
    {
      //hist->SetBinContent(j,S.at(j));
      //scope(0)->wf(0)->SetBinContent(j,S.at(j));
      scope(0)->wf(0)->SetBinContent(j,Q.at(j));
    }
  //hist->Draw();
  //Q.clear();
  //R.clear();
