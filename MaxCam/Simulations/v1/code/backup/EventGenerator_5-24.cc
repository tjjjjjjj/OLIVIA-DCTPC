...
int step(int& jmin, double& En, double dx, double& t, double dt, double a1, double a2, int i, double vd, int jmax, int jmax_init, vector<double>& Q, vector<double>& fRecoilZ, vector<double>& fRecoilEn, TRandom3* rand) //Computes when the electrons hit the mesh.
{
  ...
  jD = j(t,sz,vd,dt);
  int Nsamples = 50; //small = noisy data. large = slow.
...
{ 
  ...
  */
  ...
  const int jmax_init = 20000; //size of memory allocated when vectors need more space. Should be much larger than random noise.
 ...
 {
   need_space = step(jmin,En,dx,t,dt,a1,a2,i,vd,jmax,jmax_init,Q,fRecoilZ,fRecoilEn,rand);
  ...
  int plotmin = presize; 
  //Discretizes the waveform to the scope's resolution

 for (int j = plotmin; j < plotmax; j++) 
   scope(0)->wf(0)->SetBinContent(j,round(Scale*S.at(j - (plotmin - jmin) - zero_offset),scoperes));
