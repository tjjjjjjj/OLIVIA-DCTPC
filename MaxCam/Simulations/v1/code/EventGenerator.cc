...
void decay(int decay_offset, double Tr, double Td, double dt, vector<double>& Q, vector<double>& R, int jmax) //simulates rise, decay times
{
  int risesteps = int(Tr/dt) + 1;
  double DecayConst = 1-std::exp(-dt/Td);
  vector<double> B(risesteps,0.); //used as a "buffer" for the rise time
	...
	/*R.at(i) = R.at(i-1);
	if (i>=decay_offset)
	  R.at(i) -= DecayConst*R.at(i-decay_offset);
	  R.at(i) += vecsum(B,risesteps)/risesteps; *//*Incorrect*/
	R.at(i) = (1-DecayConst)*R.at(i-1)+vecsum(B,risesteps)/risesteps;
      }
}
...
{ 
  ...
  */
  ...
  double zero_offset_var = scope(0)->getZeroOffsetVar();
  zero_offset += int(zero_offset_var*(a1+a2*pow(fRecoilZ.at(0),0.5)));
  int Nsamples = scope(0)->getNSamples();
  double vertical_offset = scope(0)->getVerticalOffset() * 1/Scale;
  int decay_offset = scope(0)->getDecayOffset();
  ...
  for(int i=imin;En>fMinEnergy&&i<imax-1&&fRecoilZ.at(i)>0&&fRecoilZ.at(i)<fChamber->getDriftLength();i++) //main simulation; generates Q (electron arrival at mesh)
 {
  ...
  decay(decay_offset,Tr,Td,dt,Q,R,jmax); //generates R (output after rise/decay)
