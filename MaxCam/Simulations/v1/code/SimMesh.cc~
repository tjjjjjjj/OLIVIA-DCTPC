...
void SimMesh::simulate()// simulates waveforms on an initialized instance of SimMesh
{
  ...
  for(int i = imin; En>fMinEnergy && i < imax-1 && fRecoilZ.at(i)>0 && fRecoilZ.at(i)<fChamber->getDriftLength(); i++)
    ...
    {
      final_position = i+1; // after the end of this "for" loop, final_position will be the last value of i used in the loop.
...
void SimMesh::step(int i, TH1D* firsthist) //runs one step of electron ionization and drift
{
  ...
  jD = j(sz);
  jI = int(jD+.5);
  if (i%20 == 0) MakeGaussian(ker,tSD,Nsamples); //This is the most CPU-intensive part of the code, but isn't very sensitive to a small time step, so it makes sense to run it only once every few steps.
  int vecsize = (ker.size()-1)/2;

  jmin = (jI - vecsize < jmin) ? jI-vecsize : jmin; //Keep track of the lowest bin in which a signal is recorded
  if (jI + vecsize + 1000 >= jmax) need_space = 1; //if the electrons are getting near the end of the vector that is recording them
  firsthist->SetBinContent(jI,firsthist->GetBinContent(jI)+El); //add on the new batch of electrons to the signal
...
{double offset;
  if (x <0.)
    offset = -.5; //The +/- .5 is needed to actually round x, instead of just truncating it
  if (x==0.)
    offset =  0.;
  if (x >0.)
    offset =  .5;
