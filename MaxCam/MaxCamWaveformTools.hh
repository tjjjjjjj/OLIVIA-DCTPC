#ifndef MAXCAM_Waveform_TOOLS_HH
#define MAXCAM_Waveform_TOOLS_HH
...
class MaxCamWaveformTools :public TObject {
...
public:
   ...
   */
    MaxCamWaveformTools(TH1F *wf, bool doInvert=false,
   ...
   /** takes y->-y for all bins for the TH1 */
    ...
    void fillPulse(TH1F* wfi, MaxCamPulse* pulse);
    void getCopyOfPulseList(list<MaxCamPulse>& list){ list.resize(_pulseList.size()); copy(_pulseList.begin(),_pulseList.end(),list.begin()); }
    Int_t getSizeOfPulseList(void){ return _pulseList.size(); }

    MaxCamPulse returnPulseInPulseList(Int_t i){
      cout << "i=" << i << endl;
      cout << "getSizeOfPulseList()=" << getSizeOfPulseList() << endl;

      if(i<=getSizeOfPulseList()){
	list<MaxCamPulse>::iterator iter=_pulseList.begin();
	for(Int_t j=1; j<i; ++j){
...
void histOfFunc(TH1F* hin, TF1* fxn);
...
private:
   ...
   float _threshold;
   MaxCamPulse* _maxPulse;
   MaxCamPulse* _minPulse;
   list< MaxCamPulse > _pulseList;
   ...
   ClassDef(MaxCamWaveformTools,0)
};
