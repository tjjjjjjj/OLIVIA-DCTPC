...
ClassImp(DmtpcWaveformTools)


DmtpcWaveformTools::DmtpcWaveformTools() {;}
DmtpcWaveformTools::~DmtpcWaveformTools() {;}


DmtpcWaveformTools::DmtpcWaveformTools(TH1F *wf, bool doInvert,
...
void
DmtpcWaveformTools::evaluate(int nbaselinebins, int nrebin) {
   ...
   
   {
     ...
     // fill a DmtpcPulse object for the waveform minimum
     _minPulse = new DmtpcPulse(minbin);
...

   {
     ...
     // into our class' list of DmtpcPulses...
     findAllPeaks(wfi);
   }
...
void
DmtpcWaveformTools::invertPolarity() {
    int n=_wf->GetNbinsX();
...
void
DmtpcWaveformTools::print() {

  
  cout << "********************************************************" << endl;

  cout << "** void DmtpcWaveformTools::print(): " << endl;
...
TH1F*
DmtpcWaveformTools::smooth(TH1F* hdata, TH1F* hkernel) {
  ...
  //DmtpcWaveformTools::convolve(f, ndata, g, nkernel, h);
...
TH1F*
DmtpcWaveformTools::makeGaussKernel(float binWidth, float sigma, float nsigma) {
...
void 
DmtpcWaveformTools::convolve(double ff[], int nf, double gg[], int ng, double hh[]) {
...
TH1F*
DmtpcWaveformTools::convolve(TH1F* hdata, TH1F* hkernel, bool flip, bool draw) {
...
TGraph*
DmtpcWaveformTools::graphOfHist(TH1* h1, TString grName, bool flip) {
...
Double_t
DmtpcWaveformTools::calculateBaseline(TH1F* wfi, int nbaselinebins) {
...
Double_t
DmtpcWaveformTools::calculateBaselineRMS(TH1F* wfi, int nbaselinebins) {
 ...
 Double_t
 DmtpcWaveformTools::calculateIntegral(TH1F* wfi, Int_t startBin, Int_t endBin) {
...
void DmtpcWaveformTools::findAllPeaks( TH1F* wfi ){
  ...
  for( list< pair<Double_t,Double_t> >::iterator iter = wf_vordered.begin();
	...
	for (Int_t ii=1; ii<wfi->GetNbinsX()+1; ++ii){
	  ...
	  if( TMath::Abs(wfi->GetBinCenter(ii)-this_time)<1.e-20 ){
	    ...
	    startOfPulse=findPulseStart(wfi,ii,false);
	    if(startOfPulse==-1) isLocalMinimum=false;
	    
	    // find the pulse end
	    endOfPulse=findPulseEnd(wfi,ii,false);
  ...
  if(debug)  cout << "DmtpcWaveformTools::findAllPeaks(): Found " << foundPulseList.size() << " negative pulses..." << endl;
  
  // loop over the found peaks and shunt them into DmtpcPulse objects...
  for(list< pair<Double_t,Double_t> >::iterator foundPulseIter = foundPulseList.begin(); foundPulseIter!=foundPulseList.end(); foundPulseIter++){
    ...
    DmtpcPulse* this_pulse=new DmtpcPulse(wfi->FindBin((*foundPulseIter).second));
...
void DmtpcWaveformTools::fillPulse(TH1F* wfi, DmtpcPulse* pulse){
  
  //  cout << "in void DmtpcWaveformTools::FillPulse(DmtpcPulse* pulse)..." << endl;
  ...
  //  cout << "leaving void DmtpcWaveformTools::FillPulse(DmtpcPulse* pulse)..." << endl;
}
   
// returns -1 if isLocalMinimum is false
Double_t DmtpcWaveformTools::findPulseStart(TH1F* wfi, Int_t pulseBin, Bool_t debug ){
  
  Bool_t isLocalMinimum=true;
  Double_t startOfPulse=wfi->GetBinCenter(pulseBin); // it's at least this
...
// returns -1 if isLocalMinimum is false
Double_t DmtpcWaveformTools::findPulseEnd(TH1F* wfi, Int_t pulseBin, Bool_t debug ){

  Bool_t isLocalMinimum=true;
  Double_t endOfPulse=wfi->GetBinCenter(pulseBin); // it's at least this
