...
void calculaterayleigh(int n, double* angles, bool axial,		       double& Rbar2, double& phibar, double& Rstar)
...
//    TString origpathbase = "/net/zwicky/dmtpc/data/10L/dmtpc_10L_";
   ...
   for(int i=0; i<2; i++)
   {
      ...
      name = "hE";
      name+=i;
      hE[i]=new TH1F(name,name,100,0,300);
      ...
      name="hRvE";
      name+=i;
      hRvE[i]=new TH2F(name,name,120,0,300,32,0,8);
...
//	 cameraMap[d.event()->cameraSerialNumber(1)]=1;
	 ...
	 _Erecoil[events[i][2]][events[i][3]] < 300 )
      {
...
//	 time->Set(int(rnd->Uniform(lowtime,hightime)),1,0,0);
   ...
   for(int i=0; i<=hRvE[0]->GetNbinsX(); i++)
   {
      for(int j=1; j<=hRvE[0]->GetNbinsY(); j++)
      {
	 if(hRvE[0]->GetBinContent(i,j))
	    hRvE[0]->SetBinContent(i,j,1);
      }
   }

   hRvE[0]->SetFillColor(kGray);
   ...
   calculaterayleigh(camphi[0].size(),&rawphiax[0][0],false,Rbar2,phibar,Rstar);   
   cout << "Raw Phi Axial: " << endl << "\t Rbar2: " << Rbar2 << "\t phibar: " << phibar	
	<< "\t Rstar: " << Rstar << "\t Prob: " << TMath::Prob(Rstar,2) << endl;   
   calculaterayleigh(camphi[0].size(),&azimuth[0][0],false,Rbar2,phibar,Rstar);      
   cout << "Azimuth: " << endl << "\t Rbar2: " << Rbar2 << "\t phibar: " << phibar	
	<< "\t Rstar: " << Rstar << "\t Prob: " << TMath::Prob(Rstar,2) << endl;   
   calculaterayleigh(camphi[0].size(),&redphi[0][0],false,Rbar2,phibar,Rstar);      
   cout << "Reduced Phi: " << endl << "\t Rbar2: " << Rbar2 << "\t phibar: " << phibar	
	<< "\t Rstar: " << Rstar << "\t Prob: " << TMath::Prob(Rstar,2) << endl;   
   calculaterayleigh(camphi[0].size(),&redphiax[0][0],true,Rbar2,phibar,Rstar);      
   cout << "Reduced Phi: " << endl << "\t Rbar2: " << Rbar2 << "\t phibar: " << phibar	
	<< "\t Rstar: " << Rstar << "\t Prob: " << TMath::Prob(Rstar,2) << endl;
