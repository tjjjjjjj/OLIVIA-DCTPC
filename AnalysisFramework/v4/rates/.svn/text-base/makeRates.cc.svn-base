...
//check if the centroid of a cluster is IN the cluster 
...
// distance of nearest approach to a spacer
...
// is there an edge spark that might have left an RBI?
...
// takes arguments of file with list of run # to be processed (file names hardcoded),
// configuration file
...
//	 skimfile+="_mixedskim.root";
   ...
   //set up play files for output

   TString playname = flistfile;
   playname.ReplaceAll(".txt",".play");
   ofstream play;
   play.open(playname);
   playname.ReplaceAll(".play","cutoff.play");
   ofstream cutoff;
   cutoff.open(playname);

   playname.ReplaceAll("cutoff.play","nonworm.play");
   ofstream nonwormf;
   nonwormf.open(playname);

   playname.ReplaceAll("nonworm.play","nonedge.play");
   ofstream nonedgef;
   nonedgef.open(playname);
	 ...
	 for(int t=0; t<15; t++)
	 {
	       ...
	       && ev->maxpixel(c,t)/ev->E(c,t)<0.225)
	    {
	       ...
	       hRegion[cameraMap[camSerNum]]->Fill(_Erecoil[c][t],
						    _calibratedrange[c][t]);
	       nonedgef << "10L " << ev->runNumber() << " " << ev->eventNumber() 
			<< " "  << c << " " << t << endl;
	    ...
	    if(nonZero->passes(c,t) && spark->passes(c,t) && rbi->passes(c,t) &&
	       ...
	       && ev->maxpixel(c,t)/ev->E(c,t)<0.225
	       && _Erecoil[c][t]>50 && _Erecoil[c][t]<600 
	       && ev->transverse_moment(c,2,t)>20)
	    {
	       inrange[cameraMap[camSerNum]]++;
//	       cout << _Erecoil[cameraMap[camSerNum]][t] << endl;
	       ...
	       if((!cutoffCheck(ev,c,t)) && checkCentroid(ev,c,t))
	       {
				    ...
				    10,2,(DmtpcGainMap*)gainMaps->At(cameraMap[camSerNum])))
		  {
			...
			if(distToSpacer(ev,c,t,(DmtpcGainMap*)gainMaps->At(cameraMap[camSerNum]))>=2)
			{
...
//				<< " "  << c << " " << t << endl;
	       ...
	       {
		  cutoff << "10L " << ev->runNumber() << " " << ev->eventNumber() 
			 << " "  << c << " " << t << endl;
	       }
	    ...
	    if(nonZero->passes(c,t) && spark->passes(c,t) && rbi->passes(c,t) &&
	       _wormclass[cameraMap[camSerNum]][t] > wormcutoff[cameraMap[camSerNum]]  && edge->passes(c,t) 
	       && region->passes(c,t)  && ev->maxpixel(c,t)<400 && ev->cluster_rms(c,t)>10.5
	       && ev->maxpixel(c,t)/ev->E(c,t)<0.225
	       && _Erecoil[cameraMap[camSerNum]][t]>50 && _Erecoil[cameraMap[camSerNum]][t]<200
	       && ev->moment(c,2,t) > 2*ev->transverse_moment(c,2,t))
	    {
	       momentratio[cameraMap[camSerNum]]++;
	       hMoment[cameraMap[camSerNum]]->Fill(_Erecoil[cameraMap[camSerNum]][t],
						    _calibratedrange[cameraMap[camSerNum]][t]);
	    }
	    if(nonZero->passes(c,t) && spark->passes(c,t)/*&& ev->EGainMap(c,t)/Ecal[c] > 250 && rbi->passes(c,t) &&*/
	       /*&& _wormclass[cameraMap[camSerNum]][t] > wormcutoff[cameraMap[camSerNum]]  /*&& edge->passes(c,t)*/ 
	       /*&& region->passes(c,t)*/  )
	    {
	       if(ev->ntracks(c)>=1 && !edge->passes(c,t)  && rbi->passes(c,t) &&
	       _wormclass[cameraMap[camSerNum]][t] > wormcutoff[cameraMap[camSerNum]] 
		  &&  ev->maxpixel(c,t)<400 && ev->cluster_rms(c,t)>10.5  && ev->maxpixel(c,t)/ev->E(c,t)<0.225
		  /*&& ev->y(c,t)<250 && (ev->x(c,t)>500 && ev->x(c,t)<700)*/)
	       {

//		  hAlpha[cameraMap[camSerNum]]->Fill(ev->EGainMap(c,t)/(Ecal[c]),
//						     ev->range(c,t)*rangecal[c]);	
		  hAlpha[cameraMap[camSerNum]]->Fill(ev->EGainMap(c,t),
 						     ev->range(c,t));	
		  hRvEC[cameraMap[camSerNum]]->Fill(_Erecoil[c][t],
 						    /*_calibratedrange[c][t]*/ev->range(c,t)*rangecal[c]);
		  hPhivE[cameraMap[camSerNum]]->Fill(ev->EGainMap(c,t),
 						     ev->phi(c,t));	
   ...
   //print results (hard coded for 2 cameras)
...
//	<< "Moment: \t\t" << momentratio[0] << "\t" << momentratio[0]/float(nospark[0]) 
//	<< "\t" << momentratio[1] << "\t" << momentratio[1]/float(nospark[1]) << "\n"
	<< "Charge: \t" << hascharge[0] << "\t" << hascharge[0]/float(nospark[0]) 
	<< "\t" << hascharge[1] << "\t" << hascharge[1]/float(nospark[1]) << "\n";
//	<< "Alpha: \t\t" << isalpha[0] << "\t" << isalpha[0]/float(nospark[0]) << "\t"
//	<< isalpha[1] << "\t" << isalpha[1]/float(nospark[1]) << "\n";
