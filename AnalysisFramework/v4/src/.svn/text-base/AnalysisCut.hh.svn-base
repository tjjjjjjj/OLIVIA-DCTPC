...
class AnalysisCut : public TNamed
{
   ...
   public:
	...
	Default destructor
	 ...
	 \param t track index
      */
      vector < vector <int> > passes() {return _pass;}
      ...
      */
      ...
      /** set whether the function runs once per camera
	  \param once true if run once per camera
      */
      void setOnce(bool once) {_once=once;}
      /** get whether the function runs once per camera
	  \return true if run once per camera
      */
      bool getOnce() {return _once;}
   ...
   private:
      ...
      vector < vector <int> > _pass; //nested vector of passes in cam, track
      int _type; //! type of cut (double* = 0, vector< void* >=1)
      bool _once; //! do the cut once(=true) or for every track. default is false
