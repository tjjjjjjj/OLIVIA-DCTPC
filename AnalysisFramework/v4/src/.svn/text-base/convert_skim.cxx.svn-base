              ...
              DmtpcSkimDataset & sd, bool delete_intermediate, CleanSkimConfig * conf, unsigned algo_index)
{
  ...
  TString newtmpfilename("/bias");
  newtmpfilename += tmpoutfilename;
  cout << newtmpfilename << endl;

  /* Define Temporary out files and trees */
  TFile* tmpoutfile = new TFile(newtmpfilename,"RECREATE");
  TTree* tmpskimtree;
  TH2F* tempimg=0;
  TH2F* os_tempimg=0;
 ...
 cout << "Loading complete" << endl;
  ...
  //cout << "Temporary tree created."  << endl;
  ...
  gROOT->cd();
  TString biasoutfilename;
  /* Open up file for bias frame saving */
  TFile * biasoutfile;
  if (algo_index == 0)
  {
    biasoutfilename ="/bias";
    biasoutfilename+= TString(sd.getFileName()).ReplaceAll(key+".root","bias.root");
    biasoutfilename.ReplaceAll(key+".root","bias.root");
    biasoutfile = new TFile(biasoutfilename,"RECREATE");
  cout << "Bias file created: " << biasoutfilename << endl;
  ...
  if (delete_intermediate)
    {
      if (dbg_level & dbg_progress) std::cout << "Deleting " << newtmpfilename << std::endl;
  ...
  /* Handle keys */
   ...
   const char* file1;
	 ...
	 cout << "Skimming file:  " << fullname << endl;
      ...
      //sd.getEvent(0);
      //cout << "phi: " << sd.event()->phi(0,0) << endl;
      //sd.closeRootFile();
      /* TString mvcomm("mv ");
       mvcomm += outdir;
       mvcomm += outfilename;
       mvcomm += finaldir;
       gSystem->Exec(mvcomm);*/
      /** Loop over tracks found within single image file **/
      sd.getEvent(0);
      for(Int_t numtracks=0; numtracks < sd.event()->ntracks(0); numtracks ++)
	{
	  cout << "Phi for track " << numtracks << "is: " << sd.event()->phi(0,numtracks) << endl;
	  phi.push_back(sd.event()->phi(0,numtracks));
	  radii.push_back(0.99);
	}
       
       refreshPhiHistogram(phi,radii,phiHistogram,canvas);
       }
     }
     }
   return return_val;
