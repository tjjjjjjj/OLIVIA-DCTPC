...
#include "TLatex.h"
#include "TLegend.h"
//#include "/Users/jocelyn/dark_matter/projects/DarkMatter/MaxCam/Tools.hh"
//#include "/Users/jocelyn/dark_matter/projects/DarkMatter/MaxCam/MaxCamWIMP.hh"
#include "TRolke.h"
...
// make npoints 10 to run full comparison with me
void SurfaceRunSensitivityCF(const int npoints=10, const double minDMmass=20.) {

  // Init physics constants
  double rho_D = 0.4; // DM halo density in GeV / cm3
  double v_0 = 230;   // Average DM velocity in km/sec
  ...
  double m_DM[npoints] ; 
  double x_SI_DM[8][npoints]; 
  double x_SD_DM[8][npoints]; 
  double x_SD_DM_N[8][npoints]; 
  double limit[npoints];

  for (int selectExperiment=4; selectExperiment<8; selectExperiment++) {
    ...
    if (selectExperiment==0) { 
      ...
      spinFactor =0.647 ; // lambda^2*J(J+1) 
      fractionUsableSD = 0.864; // 12(C)/(12+4*19(F)) 
      activeMass = 0.0033; // total mass in Kg -- 1m3@50 torr = 0.250 kg 
      upTime = (2135795*pow(24*3600,-1))/2.;     // up time in days 
      detEfficiency = 0.70 ; // detector efficiency 
      E1=80.0;  E2=200.0; // new cuts Lower and upper thresholds in keV
      ...
      spinFactor =0.647 ; // lambda^2*J(J+1) 
      fractionUsableSD = 0.864; // 12(C)/(12+4*19(F)) 
      activeMass = 0.0033; // mass in Kg -- 1m3@50 torr = 0.250 kg 
      // original
      //      upTime = (2135795*pow(24*3600,-1))/2.;     // up time in days 
      upTime=13.33; // to match my exposure number for Fluorine
      //      detEfficiency = 0.70 ; // detector efficiency 
      detEfficiency = 0.7 ; // detector efficiency 
      E1=90.0;  E2=200.0; // new cuts Lower and upper thresholds in keV
      R0_measured=53;        // Measured rate  
      Exp_background = 0;
      nUnpairedProtons  = 1;  nUnpairedNeutrons = 0; 

    } else if (selectExperiment==5) {  // DMTPC underground run (59.9 gm-day), 0 exp. bgnd
      
      ExpName = "DMTPC 10L Underground Run, 0 Expected Bgnd, 1s assumption";   
      Atarget =19. ; // mass target recoil in GeV c^2 (F=19, Xe=131, Ge=72)
      spinFactor =0.647 ; // lambda^2*J(J+1) 
      fractionUsableSD = 0.864; // 12(C)/(12+4*19(F)) 
      activeMass = 0.002615; // mass in Kg -- 1m3@50 torr = 0.250 kg 
      // original
      //      upTime = (2135795*pow(24*3600,-1))/2.;     // up time in days 
      upTime=22.755; // to match my exposure number for Fluorine
      //      detEfficiency = 0.70 ; // detector efficiency 
      detEfficiency = 1 ; // detector efficiency 
      E1=80.0;  E2=200.0; // new cuts Lower and upper thresholds in keV
      R0_measured=781;        // Measured rate  
      Exp_background = 0;
      nUnpairedProtons  = 1;  nUnpairedNeutrons = 0; 

    }
    else if (selectExperiment==6) {  // DMTPC underground run (59.9 gm-day), 0 exp. bgnd
      
      ExpName = "DMTPC 10L Underground Run, 0 Expected Bgnd, 1.3s assumption";   
      Atarget =19. ; // mass target recoil in GeV c^2 (F=19, Xe=131, Ge=72)
      spinFactor =0.647 ; // lambda^2*J(J+1) 
      fractionUsableSD = 0.864; // 12(C)/(12+4*19(F)) 
      activeMass = 0.002615; // mass in Kg -- 1m3@50 torr = 0.250 kg 
      // original
      //      upTime = (2135795*pow(24*3600,-1))/2.;     // up time in days 
      upTime=29.582; // to match my exposure number for Fluorine
      //upTime=4.18402; // to match my exposure number for Fluorine
      //      detEfficiency = 0.70 ; // detector efficiency 
      detEfficiency = 1 ; // detector efficiency 
      E1=80.0;  E2=200.0; // new cuts Lower and upper thresholds in keV
      //R0_measured=30.838361;        // Measured rate  
      R0_measured=781;        // Measured rate  
      Exp_background = 0;
      nUnpairedProtons  = 1;  nUnpairedNeutrons = 0; 

    }/*else if (selectExperiment==6) {  // DMTPC 4-shooter at WIPP
      
      ExpName = "DMTPC 20L WIPP 1-year";   
      Atarget =19. ; // mass target recoil in GeV c^2 (F=19, Xe=131, Ge=72)
      spinFactor =0.647 ; // lambda^2*J(J+1) 
      fractionUsableSD = 0.864; // 12(C)/(12+4*19(F)) 
      activeMass = 0.0081; // mass in Kg -- 1m3@50 torr = 0.250 kg 
      upTime = 365.;     // up time in days 
      detEfficiency = 0.5 ; // detector efficiency 
      E1=80.;  E2=200.0; // Lower and upper thresholds in keV
      R0_measured=0.;        // Measured rate  
      //      Exp_background = R0_measured;
      Exp_background = 0.;
      nUnpairedProtons  = 1;  nUnpairedNeutrons = 0; 

      }*/ else if (selectExperiment==7) {  // DMTPC 1m3 at WIPP
      ...
      fractionUsableSD = 0.864; // 12(C)/(12+4*19(F)) 
      activeMass = 0.002615; // mass in Kg -- 1m3@50 torr = 0.250 kg 
      upTime = 4.18402;     // up time in days 
      detEfficiency = 1 ; // detector efficiency 
      E1=80.;  E2=200.0; // Lower and upper thresholds in keV
      R0_measured=30.838;        // Measured rate  
      //      Exp_background = R0_measured;
      Exp_background = 0.;
      nUnpairedProtons  = 1;  nUnpairedNeutrons = 0; 
    ...
    if (selectExperiment==4) {
      limitRate = CL90(R0_measured); // set 90% CL limit no background 
    } else {
    ...
    cout << "Exposure*fractionUsableSD=" << exposure*fractionUsableSD << " kg*days"<< endl; 
       
    //    for (int i=0;i<npoints;i++) { 
    for( Int_t i=0; i<npoints; ++i){
      //      double MD =  minDMmass+i*1. ; 
      double MD =  minDMmass*TMath::Exp(0.4*i);
      //      double MD=110;
      m_DM[i]= MD; 
      ...
      cout << "R0_eff_corr=" << R0_eff_corr << endl;
      double R0 = R0_eff_corr * c2 / c1 / (exp(-c2*E1/(E0*r)) - exp(-c2*E2/(E0*r)) ) ; 
      //      R0/=spinFactor;
      //      R0/=CWp2;
      cout << "(exp(-c2*E1/(E0*r)) - exp(-c2*E2/(E0*r)) )=" << (exp(-c2*E1/(E0*r)) - exp(-c2*E2/(E0*r)) ) << endl;
      cout << "c2 / c1 / (exp(-c2*E1/(E0*r)) - exp(-c2*E2/(E0*r)) )=" << c2 / c1 / (exp(-c2*E1/(E0*r)) - exp(-c2*E2/(E0*r)) ) << endl;
      cout << "R0=" << R0 << endl;
    ...
    if(selectExperiment==5 ||selectExperiment==6 ||selectExperiment==7 ){
  ...
  TGraph *graphSD_dmtpc_wipp_4sh = new TGraph(npoints, m_DM, limit );  


  for (int i=0; i<npoints; i++) {
    limit[i]=x_SD_DM[7][i];
  }
  TGraph *graphSD_dmtpc_5swipp = new TGraph(npoints, m_DM, limit );  
  ...
  graphSD_newage_surface->SetLineStyle(4);
  graphSD_dmtpc_surface_bgndsub->SetLineStyle(1);
  graphSD_dmtpc_surface_bgndsub->SetLineColor(17);
  graphSD_dmtpc_surface->SetLineStyle(2);
  graphSD_dmtpc_surface->SetLineColor(15);
  graphSD_dmtpc_wipp->SetLineStyle(1);
  graphSD_dmtpc_wipp->SetLineColor(1);
  graphSD_dmtpc_wipp_4sh->SetLineStyle(3);
  graphSD_dmtpc_wipp_4sh->SetLineColor(4);
  graphSD_dmtpc_5swipp->SetLineStyle(9);
  graphSD_dmtpc_5swipp->SetLineColor(6);
  ...
  graphSD_dmtpc_wipp_4sh->SetLineWidth(4);  
  graphSD_dmtpc_5swipp->SetLineWidth(4);  
  ...
  g_picasso->SetLineWidth(4);  g_picasso->SetLineStyle(2);  
  g_picasso->SetLineColor(7); 
  g_picasso->Draw("C"); 
  ...
  g_kims->SetLineWidth(4);  g_kims->SetLineStyle(2);  
  ...
  legend->AddEntry("graphSD_dmtpc_surface","DMTPC 10L Surface Run 2009","l");
  graphSD_dmtpc_wipp->SetName("graphSD_dmtpc_wipp");
  legend->AddEntry("graphSD_dmtpc_wipp","DMTPC 10L Underground Run 2011, 1s, 22.775 live days","l");
  graphSD_dmtpc_wipp_4sh->SetName("graphSD_dmtpc_wipp_4sh");
  legend->AddEntry("graphSD_dmtpc_wipp_4sh","DMTPC 10L Underground Run 2011, 1.3s, 29.582 live days","l");
...
//  legend->AddEntry("g_coupp","COUPP 2008","l");
   graphSD_dmtpc_5swipp->SetName("graphSD_dmtpc_5swipp");
   legend->AddEntry("graphSD_dmtpc_5swipp","DMTPC 10L Underground Run 2011, 5s, 4.18 live days","l");
//   g_th->SetName("g_th");
...
//  cout << endl;
  ...
  TFile f("current_limit_analysis.root","RECREATE");
  graphSD_dmtpc_surface->Write();
  f.Close();
}
