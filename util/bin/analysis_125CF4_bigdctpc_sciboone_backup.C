...
{	
	  if (ev2->maxpixel(0,k)>1500||ev2->npixel(0,k)>1500. || ev2->image_rms(0)<10.5){
...
time_start=ev->timenow(0);
	...
	for(int k=0;k<ev->ntracks(0);k++)
	  {	
	  if (ev->maxpixel(0,k)>1500||ev->npixel(0,k)>1500. || ev->image_rms(0)<10.5){lastSpark = ev->eventNumber();continue;}//maxpixel has to be less than 1000ADU. also, the event might have been a spark! also, sparks tend to have a lot of pixels.
	  }
