...
void analysis_125CF4_bigdctpc_alpha_wf(int firstrun, int lastrun){//inclusive 
  ...
  TFile *histfile=new TFile(Form("outtree_%d_%d_wf.root",firstrun,lastrun), "RECREATE");
  ...
  tree2->Branch("SeriesNum", &sernum, "sernum/I");
...
time_start=ev->timenow(0);
	...
	for(int k=0;k<ev->ntracks(0);k++)
	  {	
	  if (ev->maxpixel(0,k)>800){lastSpark = ev->eventNumber();continue;}//maxpixel has to be less than 800ADU. also, the event might have been a spark!
	  }
...
// 	    else continue;
	...
	CspWfVector* veto = (CspWfVector*) arr->At(2); 
	    ...
	    diff = TMath::Abs( ((mesh->at(nt,0).getIntegral()*meshCalib) - E2) / ((mesh->at(nt,0).getIntegral()*meshCalib) + E2) );

	    //cout<<mesh->at(nt,0).getIntegral()*meshCalib<<" "<<E2<<endl;
	...
	passAll=1;
	//	cout<<"Chosen: "<<Emesh<<" "<<Etrig<<endl;
	//	cout<<"-----------------"<<endl;
	tree->Fill();	
			//	cout<<bestDiff<<" "<<diff<<" "<<E2<<" "<<Etrig<<" "<<Emesh<<endl;
}//end event loop
