...
#include <iomanip>;
#include <sys/stat.h>;
void analysis_125CF4_bigdctpc_alpha_sim(int firstrun, int lastrun){//inclusive 
  ...
  int BATCH=1;//0 if running interactively, 1 if running batch
...
long int new_filesize2=0;
  ...
  TFile *histfile=new TFile(Form("outtree_sim_%s_%s.root",out1filename.c_str(),out2filename.c_str()), "RECREATE");
...
totaltrig=0;
      ...
      if(BATCH==0)
      {
      string origfile = "/net/nudsk0001/d00/scratch/spitzj/bigdctpc_sim_out/myOutput_";
      string skimfile = "/net/nudsk0001/d00/scratch/spitzj/bigdctpc_sim_out/myOutput_";    
	  }
	  else
	  {
