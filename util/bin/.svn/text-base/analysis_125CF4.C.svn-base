void analysis_125CF4(int firstrun, int lastrun){//inclusive 
  ...
  double chargeCalib = 6.5;
  double meshCalib = 3.3;
  ...
  TH2D *hist_energy_range_ccd=new TH2D("Energy Range CCD","Energy Range CCD",50,10,1000,50,0,15);
  ...
  for (int x = firstrun; x <= lastrun; x++)
	...
	for(int k=0;k<ev->ntracks(0);k++)
	  {	    
	    ...
	    if (ev->maxpixel(0,k)>250){cutnum=44; hist_cutnum->Fill(cutnum); continue;} //maxpixel has to be less than 250ADU
	    ...
	    for (int nt = 0; nt < mesh->size(); nt++){	  
	      ...
	      if (mesh->at(nt).getRMS() >0.005){cutnum=1; continue;}//for neutron calib run: >0.002
	      if (anode->at(nt).getRMS()>0.0007){cutnum=2; continue;}//for neutron calib run: >0.003
	      ...
	      E2 = (anode->at(nt,0).getPeak()*1000)*chargeCalib; //creation of E_anode
	      E3 = (mesh->at(nt,0).getPeak()*1000)*meshCalib;
		...
		Emesh = E3;
	      ...
	      cout << "Etrack: " << Etrack << endl << "Etrig: " << Etrig << endl << "Emesh: "<<Emesh<< endl <<"bestdiff: " << bestDiff<< endl;
	    } // end trigger loop 
	    ntotalTrig+=passTrig;
  ...
  hist_energy->SetYTitle("E_{anode} (keV)");
  hist_energy->Draw("COLZ");

  new TCanvas; 
  hist_energy2->SetTitle("Anode Energy vs. Mesh Energy");
  hist_energy2->SetXTitle("E_{anode} (keV)");
  hist_energy2->SetYTitle("E_{mesh} (keV)");
  hist_energy2->Draw("COLZ");

  new TCanvas; 
  hist_energy3->SetTitle("CCD Energy vs. Mesh Energy");
  hist_energy3->SetXTitle("E_{CCD} (keV)");
  hist_energy3->SetYTitle("E_{mesh} (keV)");
  ...
  hist_energy_anode->SetXTitle("E_{anode} (keV)");
  ...
  hist_diff->SetXTitle("E_{CCD} - E_{anode} (keV)");
  hist_diff->Fit("gaus");
