...
void analysis_125CF4_dctpc_alpha_wf(int firstrun, int lastrun){//inclusive 
...
// 	    else continue;
	...
	CspWfVector* veto = (CspWfVector*) arr->At(2);; 
	    ...
	    //diff = (mesh->at(nt,0).getIntegral()*meshCalib) - E2;
	    
	    diff=TMath::Abs( ((mesh->at(nt,0).getIntegral()*meshCalib) - E2) / ((mesh->at(nt,0).getIntegral()*meshCalib) + E2) );
	    
	    //cout<<evnum<<" "<<(mesh->at(nt,0).getIntegral()*meshCalib)<<" "<<E2<<" "<<num<<endl;
