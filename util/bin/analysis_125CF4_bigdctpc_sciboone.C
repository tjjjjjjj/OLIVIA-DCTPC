...
void analysis_125CF4_bigdctpc_alpha(int firstrun, int lastrun){//inclusive 
  ...
  double anodeCalib= 69.6;
  double meshCalib = 604.8;
...
long int new_filesize2=0;
  ...
  tree->Branch("Veto_max_v", &veto_max, "veto_max/D");
  ...
  tree->Branch("Veto_starttime_samp", &veto_start, "veto_start/D");
...
// continue;
      ...
      if(runnum>=26855 && runnum<=26912)
      seqnum=32;

      if(runnum>=28442 && runnum<=29071)
	  seqnum=0;
	  
      if(runnum>=29073 && runnum<=29313)
	  seqnum=1;
      
      if(runnum>=29317 && runnum<=30711)
	seqnum=2;

      if(runnum>=30712&&runnum<=32921)
	seqnum=3;
...
{	
	  if (ev2->maxpixel(0,k)>1500||ev2->npixel(0,k)>1500. ){
...
time_start=ev->timenow(0);
	...
	for(int k=0;k<ev->ntracks(0);k++)
	  {	
	  if (ev->maxpixel(0,k)>1500||ev->npixel(0,k)>1500. ){lastSpark = ev->eventNumber();continue;}//maxpixel has to be less than 1000ADU. also, the event might have been a spark! also, sparks tend to have a lot of pixels.
	  }
...
totaltrack+=ev->ntracks(0);
	...
	beamevent=0;
	    ...
	    cout<<runnum<<" "<<evnum<<" Veto peak (V): "<<veto->at(nt,0).getPeak()<<" "<<veto->at(nt).getWfMax()<<" at time (samples): "<<veto->at(nt,0).getStartTime()<<endl;
		    if(veto->at(nt,0).getPeak()>.7 || veto->at(nt).getWfMax()>.7 )
		    {
		    beamevent=1;
	    cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
	...
	//loop over tracks in each event
	  ...
	  {	 
	    ...
	    //these two lines are only for studying waveforms. make sure to comment these out for actual physics studies!
	    if(beamevent>0)
        diff=0.;
	        ...
	        veto_start=veto->at(nt,0).getStartTime();
