//
// $Id: McDarkUrbanMscModel.cc,v 1.4 2010/12/29 20:36:26 ddujmic Exp $
// GEANT4 tag $Name:  $
//
// -------------------------------------------------------------------
//
...
McDarkUrbanMscModel::McDarkUrbanMscModel(const G4String& nam)
  : G4VMscModel(nam),
    isInitialized(false)
{
  masslimite    = 0.6*MeV;
...
McDarkUrbanMscModel::~McDarkUrbanMscModel()
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void McDarkUrbanMscModel::Initialise(const G4ParticleDefinition* p,
				   const G4DataVector&)
{
...
G4double McDarkUrbanMscModel::ComputeCrossSectionPerAtom( 
...
G4double McDarkUrbanMscModel::ComputeTruePathLengthLimit(
                             const G4Track& track,
...
G4double McDarkUrbanMscModel::ComputeGeomPathLength(G4double)
{
  lambdaeff = lambda0;
  par1 = -1. ;  
  par2 = par3 = 0. ;  
...
G4double McDarkUrbanMscModel::ComputeTrueStepLength(G4double geomStepLength)
{
...
G4double McDarkUrbanMscModel::ComputeTheta0(G4double trueStepLength,
					  G4double KineticEnergy)
{
...
void McDarkUrbanMscModel::SampleScattering(const G4DynamicParticle* dynParticle,
					 G4double safety)
{
  ...
  if (latDisplasment && safety > tlimitminfix) {
    ...
    /*    
    G4cout << "McDarkUrbanMscModel::SampleSecondaries: e(MeV)= " << kineticEnergy
	   << " sinTheta= " << sth << " r(mm)= " << r
...
G4double McDarkUrbanMscModel::SampleCosineTheta(G4double trueStepLength,
					      G4double KineticEnergy)
{
  G4double cth = 0.79 ;
  while (cth<0.8) { // make GEANT4 paths consistent with SRIM [D.D.]
...
G4double McDarkUrbanMscModel::SimpleScattering(G4double xmeanth,G4double x2meanth)
{
...
G4double McDarkUrbanMscModel::SampleDisplacement()
{
...
G4double McDarkUrbanMscModel::LatCorrelation()
{
  const G4double kappa = 2.5;
  const G4double kappami1 = kappa-1.;
